...
interface ImageItem {
  ...
  tags: string[];
  uploadDate: string;
  type?: 'icons' | 'banners'; // Aggiunto type per distinguere icone/banner
}

interface ImageGridProps {
  searchTags: string[];
  refreshTrigger: number;
  activeTab: 'icons' | 'banners';
  titleSearch: string;
}

export function ImageGrid({ searchTags, refreshTrigger, activeTab, titleSearch }: ImageGridProps) {
  ...
  const [quickDownloadSize, setQuickDownloadSize] = useState<string>("100");
  ...
  const matchesTitle = titleSearch
    ? img.originalName.toLowerCase().includes(titleSearch.toLowerCase())
    : true;

  const matchesTags =
    searchTags.length === 0 ||
    searchTags.every((tag) => img.tags.includes(tag));

  // Se `type` non è definito, consideralo valido
  const matchesTab = !img.type || img.type === activeTab;
  ...
  const fetchImages = async () => {
    try {
      ...
      // Aggiungi filtro per tab (tipo di contenuto)
      params.append('type', activeTab);
      
      // Aggiungi filtro per tags
      if (searchTags.length > 0) params.append('tags', searchTags.join(','));
      
      // Aggiungi filtro per titolo
  ...
  useEffect(() => {
    fetchImages();
  }, [searchTags.join(","), refreshTrigger, activeTab, titleSearch]);
  ...
  const handleDownload = async (
    ...
    try {
      ...
      const s = size || quickDownloadSize;
        ...
        <div className="text-center space-y-4 max-w-sm">
          ...
          <div>
            <h3 className="text-xl font-semibold text-slate-700 mb-2">
              Nessuna {activeTab === 'icons' ? 'icona' : 'banner'}
            </h3>
            <p className="text-slate-500">
              {searchTags.length > 0 || titleSearch
                ? "Modifica i filtri di ricerca"
                : `Carica ${activeTab === 'icons' ? 'la prima icona' : 'il primo banner'}`}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b border-blue-100/50 bg-white/40 backdrop-blur-sm">
        <div className="flex items-center gap-4">
          ...
          <Select
            ...
            onValueChange={setQuickDownloadSize}
          >
            <SelectTrigger className="w-24 h-8 bg-white/80 border-blue-200/50 rounded-lg text-sm">
              <SelectValue />
            </SelectTrigger>
          ...
          {filteredImages.map((img) => (
            <div key={img.id} className="group relative">
              <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover-lift border border-blue-100/30">
                ...
                {/* Nome file e tags compatti */}
                <div className="p-2">
                  {/* Nome file troncato */}
                  <div className="text-xs text-slate-600 text-center mb-1 truncate px-1">
                    {img.originalName}
                  </div>
      ...
      >
        <DialogContent className="max-w-lg p-0 rounded-3xl shadow-2xl bg-white border-blue-200/50 overflow-hidden no-animations">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <Button
                          size="sm"
                          onClick={() =>
                            handleUpdateTags(selectedImage.id, newTags)
                          }
